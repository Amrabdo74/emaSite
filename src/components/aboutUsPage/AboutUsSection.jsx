...
const AboutUsSection = () => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'aboutPage', 'content'), (snap) => {
      if (snap.exists() && snap.data().aboutUsSection) {
        setData(snap.data().aboutUsSection);
      }
    });
    return () => unsub();
  }, []);

  const aboutUs = data?.aboutUs?.[lang] || t('aboutUsSection.aboutUs');
  const mainTitle = data?.mainTitle?.[lang] || t('aboutUsSection.mainTitle');
  const subTitle = data?.subTitle?.[lang] || t('aboutUsSection.subTitle');
  const description = data?.description?.[lang] || t('aboutUsSection.description');
